...
3.  **Best subset selection + OLS** → Variable selection via AIC, fit OLS on selected variables (with hierarchy)
...
Best subset selection identified a 6-variable model as optimal based on the AIC score, achieving an adjusted $R^2$ of 99.71%, which is nearly identical to the full model's performance but with fewer terms. This indicates that some interaction terms are unnecessary and that the simplified model provides a better balance between accuracy and interpretability, reducing the risk of overfitting.
