...
6.  **Two-stage inference** → When LASSO is selected, fit OLS on LASSO-selected variables for valid confidence intervals
7.  Generate predictions with **confidence intervals** to quantify uncertainty

**Two-Stage Approach (Instructor Guidance):** When LASSO is selected as the better method: - **Stage 1 (Selection):** Use LASSO to identify important variables (non-zero coefficients) - **Stage 2 (Inference):** Fit OLS using only LASSO-selected variables to obtain valid confidence intervals
...
if(cv_ols_mean < cv_lasso_mean) {
  ...
  # Two-stage approach: Use LASSO-selected variables in OLS for inference
  ...
  # Fit OLS on LASSO-selected variables for inference (confidence intervals)
  formula_mean_lasso <- as.formula(paste("log_mean ~", paste(lasso_vars, collapse = " + ")))
  lm_mean_lasso <- lm(formula_mean_lasso, data = train)
  
  final_model_mean <- lm_mean_lasso  # Use OLS on LASSO-selected vars
  use_lasso_mean <- TRUE
}
```
...
if(cv_ols_var < cv_lasso_var) {
  ...
  formula_var_lasso <- as.formula(paste("log_variance ~", paste(lasso_vars, collapse = " + ")))
  lm_var_lasso <- lm(formula_var_lasso, data = train)
  
  final_model_var <- lm_var_lasso
  use_lasso_var <- TRUE
}
```
...
if(cv_ols_skew < cv_lasso_skew) {
  ...
  # Fit OLS on LASSO-selected variables for inference
  formula_skew_lasso <- as.formula(paste("log_skewness ~", paste(lasso_vars, collapse = " + ")))
  lm_skew_lasso <- lm(formula_skew_lasso, data = train)
  
  final_model_skew <- lm_skew_lasso
  use_lasso_skew <- TRUE
}
```
...
if(cv_ols_kurt < cv_lasso_kurt) {
  ...
  # Fit OLS on LASSO-selected variables for inference
  formula_kurt_lasso <- as.formula(paste("log_kurtosis ~", paste(lasso_vars, collapse = " + ")))
  lm_kurt_lasso <- lm(formula_kurt_lasso, data = train)
  
  final_model_kurt <- lm_kurt_lasso
  use_lasso_kurt <- TRUE
}
```
...
```{r test_predict}
# Generate predictions using final models (all are OLS models)
# Note: When LASSO was selected, final_model uses OLS on LASSO-selected variables
...
# Two-stage approach: All final models are OLS (either from subset or LASSO-selected vars)

# Mean model CIs
pred_mean_ci <- predict(final_model_mean, newdata = test, interval = "confidence", level = 0.95)
pred_mean_pi <- predict(final_model_mean, newdata = test, interval = "prediction", level = 0.95)
...
cat("=== MEAN MODEL ===\n")
cat("Average 95% CI width:", round(mean(mean_intervals$ci_width), 4), "\n")
...
# Variance model CIs
pred_var_ci <- predict(final_model_var, newdata = test, interval = "confidence", level = 0.95)
pred_var_pi <- predict(final_model_var, newdata = test, interval = "prediction", level = 0.95)

var_intervals <- test %>%
  select(St, Re, Fr) %>%
  mutate(
    pred = exp(pred_var_ci[, "fit"]),
    ci_lower = exp(pred_var_ci[, "lwr"]),
    ci_upper = exp(pred_var_ci[, "upr"]),
...
cat("\n=== VARIANCE MODEL ===\n")
cat("Average 95% CI width:", round(mean(var_intervals$ci_width), 4), "\n")
cat("Average 95% PI width:", round(mean(var_intervals$pi_width), 4), "\n")

# Skewness model CIs
pred_skew_ci <- predict(final_model_skew, newdata = test, interval = "confidence", level = 0.95)
pred_skew_pi <- predict(final_model_skew, newdata = test, interval = "prediction", level = 0.95)

skew_intervals <- test %>%
  select(St, Re, Fr) %>%
  mutate(
    pred = exp(pred_skew_ci[, "fit"]),
    ci_lower = exp(pred_skew_ci[, "lwr"]),
    ci_upper = exp(pred_skew_ci[, "upr"]),
...
cat("\n=== SKEWNESS MODEL ===\n")
cat("Average 95% CI width:", round(mean(skew_intervals$ci_width), 4), "\n")
cat("Average 95% PI width:", round(mean(skew_intervals$pi_width), 4), "\n")

# Kurtosis model CIs
pred_kurt_ci <- predict(final_model_kurt, newdata = test, interval = "confidence", level = 0.95)
pred_kurt_pi <- predict(final_model_kurt, newdata = test, interval = "prediction", level = 0.95)

kurt_intervals <- test %>%
  select(St, Re, Fr) %>%
  mutate(
    pred = exp(pred_kurt_ci[, "fit"]),
    ci_lower = exp(pred_kurt_ci[, "lwr"]),
    ci_upper = exp(pred_kurt_ci[, "upr"]),
...
cat("\n=== KURTOSIS MODEL ===\n")
cat("Average 95% CI width:", round(mean(kurt_intervals$ci_width), 4), "\n")
cat("Average 95% PI width:", round(mean(kurt_intervals$pi_width), 4), "\n")

cat("\n** Interpretation **\n")
cat("- Confidence Interval (CI): Uncertainty in the mean prediction\n")
cat("- Prediction Interval (PI): Uncertainty for a new observation (wider)\n")
cat("- Wide intervals expected since only 3 observed Re values\n")
cat("- When LASSO was selected, CIs come from OLS on LASSO-selected variables\n")
```
