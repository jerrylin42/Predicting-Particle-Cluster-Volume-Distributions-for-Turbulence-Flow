...
# Mean
pred_mean_gam <- predict(gam_mean, newdata = test, se.fit = TRUE, type = "link")
sigma_mean <- sqrt(mean(residuals(gam_mean)^2))
pred_mean_pi <- test %>%
  mutate(
    fit = exp(pred_mean_gam$fit),
    pi_lower = exp(pred_mean_gam$fit - z * sqrt(pred_mean_gam$se.fit^2 + sigma_mean^2)),
    pi_upper = exp(pred_mean_gam$fit + z * sqrt(pred_mean_gam$se.fit^2 + sigma_mean^2)),
    pi_width = pi_upper - pi_lower
  )

# Variance
pred_var_gam <- predict(gam_variance, newdata = test, se.fit = TRUE, type = "link")
sigma_var <- sqrt(mean(residuals(gam_variance)^2))
pred_var_pi <- test %>%
  mutate(
    fit = exp(pred_var_gam$fit),
    pi_lower = exp(pred_var_gam$fit - z * sqrt(pred_var_gam$se.fit^2 + sigma_var^2)),
    pi_upper = exp(pred_var_gam$fit + z * sqrt(pred_var_gam$se.fit^2 + sigma_var^2)),
    pi_width = pi_upper - pi_lower
  )

# Skewness
pred_skew_gam <- predict(gam_skewness, newdata = test, se.fit = TRUE, type = "link")
sigma_skew <- sqrt(mean(residuals(gam_skewness)^2))
pred_skew_pi <- test %>%
  mutate(
    fit = exp(pred_skew_gam$fit),
    pi_lower = exp(pred_skew_gam$fit - z * sqrt(pred_skew_gam$se.fit^2 + sigma_skew^2)),
    pi_upper = exp(pred_skew_gam$fit + z * sqrt(pred_skew_gam$se.fit^2 + sigma_skew^2)),
    pi_width = pi_upper - pi_lower
  )

# Kurtosis
pred_kurt_gam <- predict(gam_kurtosis, newdata = test, se.fit = TRUE, type = "link")
sigma_kurt <- sqrt(mean(residuals(gam_kurtosis)^2))
pred_kurt_pi <- test %>%
  mutate(
    fit = exp(pred_kurt_gam$fit),
    pi_lower = exp(pred_kurt_gam$fit - z * sqrt(pred_kurt_gam$se.fit^2 + sigma_kurt^2)),
    pi_upper = exp(pred_kurt_gam$fit + z * sqrt(pred_kurt_gam$se.fit^2 + sigma_kurt^2)),
    pi_width = pi_upper - pi_lower
  )

cat("Average 95% PI widths (GAM models):\n")
cat("Mean:", mean(pred_mean_pi$pi_width), "\n")
cat("Variance:", mean(pred_var_pi$pi_width), "\n")
cat("Skewness:", mean(pred_skew_pi$pi_width), "\n")
cat("Kurtosis:", mean(pred_kurt_pi$pi_width), "\n")
...
```{r}
yhat_train <- as.vector(predict(lasso_kurt, s = bestlam_kurt, newx = X_kurt_full))
SSE_tr <- sum((y_kurt - yhat_train)^2)
SST_tr <- sum((y_kurt - mean(y_kurt))^2)
R2_train_lasso <- 1 - SSE_tr / SST_tr
R2_train_lasso
```
