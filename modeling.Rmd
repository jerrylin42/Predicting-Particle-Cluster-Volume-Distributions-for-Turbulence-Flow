...
## 2.6 GAM (Optional - Commented Out)

```{r eval=FALSE}
# Uncomment if you want to explore GAM models
# library(mgcv)
# 
# gam_mean <- gam(
#   log_mean ~
...
### Diagnostic Plots for Variance Model

```{r var_diagnostics, fig.height=4, fig.width=8}
par(mfrow = c(1, 2))

# Residuals vs Fitted
plot(ols_proxy_var, which = 1, main = "Variance: Residuals vs Fitted")

# Q-Q Plot
plot(ols_proxy_var, which = 2, main = "Variance: Q-Q Plot")
```

------------------------------------------------------------------------

## 3.6 GAM (Optional - Commented Out)
...
## 4.6 GAM (Optional - Commented Out)
...
## 5.6 GAM (Optional - Commented Out)
...
```{r var_interval_metrics}
# More robust interval metrics for variance model
var_intervals <- var_intervals %>%
  mutate(relative_width = (pi_upper - pi_lower) / pred * 100)

cat("\n--- Variance Model Prediction Interval Metrics ---\n")
cat("Median width:", median(var_intervals$pi_width), "\n")
cat("IQR of widths:", IQR(var_intervals$pi_width), "\n")
cat("Median relative width:", median(var_intervals$relative_width), "%\n")
cat("Mean width (for comparison):", mean(var_intervals$pi_width), "\n")
```
...
## CV RMSE With GAM (Optional - Commented Out)

```{r eval=FALSE}
# Uncomment if you want to compare with GAM models
